...
int httpx::Socket::init_socket(const AddressFamilies domain,
  ...
  int fd = socket(as_integer(domain), as_integer(type), as_integer(protocol));
  if (fd < 0) {
    switch (errno) {
    case EACCES:
      throw httpxSocketException("Permission to create a socket of the "
    ...
    case EMFILE:
      throw httpxSocketException("The per-process limit on the number of open "
                                 "file descriptors has been reached.");
    case ENFILE:
      throw httpxSocketException("The system-wide limit on the total number of "
    ...
    case EPROTONOSUPPORT:
      throw httpxSocketException("The protocol type or the specified protocol "
